...
filename = "E:\\study\\计算智能\\big work\\论文\\AG_2_22.csv"
filename_2 = "E:\\study\\计算智能\\big work\\论文\\AG_2_22_MAE.csv"
steps = 2
...
class dna:
    def __init__(self):
        # 基因数组，这里为浮点数
        self.peace = np.array([np.random.randint(left[i],right[i]) for i in range(max_v)])
        # 适应度
        self.grade = None
        self.MAE = None
    ...
    def mutate(self):
        ...
        self.peace[ids] = np.random.uniform(left[ids],right[ids])
    ...
    def mutate_2(self,g,b = 3):
        ...
        if sign:
            self.peace[add] = self.peace[add] + (right[add] - self.peace[add]) * (np.random.random()*(1-g))**b
        else:
            self.peace[add] = self.peace[add] - (self.peace[add]-left[add])* (np.random.random()*(1-g))**b
...
def setting_work():
    ...
    global left, right, input_set, output_set
